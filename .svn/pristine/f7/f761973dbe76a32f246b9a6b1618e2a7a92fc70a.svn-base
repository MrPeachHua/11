...
function excelExport(){
	...
	window.location="<%=basePath%>products/order/excelTempExport.html?orderId="+orderId+"&carNumber="+carNumber+"&orderType="+11+"&orderStatus="+orderStatus+"&orderStatus="+orderStatus+"&customerPhone="+customerPhone+"&form_beginTime="+form_beginTime+"&form_endTime="+form_endTime+"&parkingName="+parkingName;
}
</script>
  </head>
  ...
  <body>
  	...
  	<form  action="<%=basePath%>products/order/listTemp.html" method="post">
	        	...
	        	<table width="100%" border="0" cellspacing="0" cellpadding="0" height="60">
			        ...
			        <td class="STYLE4">&nbsp;&nbsp;订单状态：
			        	...
			        	<!--  <input type="hidden" id="select2" value="${page.params['orderStatus'] }"/>-->
							...
							<td class="STYLE4" >车场名称：<input type="text"  name="parkingName" id="parkingName"  value='${page.params["parkingName"] }'  class="btn_tb" />
							</td>
					<!-- <td class="STYLE4">&nbsp;&nbsp;客户名称：
			        	<input type="text"  name="customerName" id="customerName" value='${page.params["customer_nickname"] }' style="width: 130px"/>
			                  ...
			                  <td align="right" class="STYLE4" id="select"><input  type="submit" value="查询" class="btn" />
			                  <input  type="button" name="reset" value="重置" onclick="reset1()" class="btn"/>
							  <security:authorize access="hasAnyRole('AUTH_EXP_0064')">
								 <input  type="button" class="btn" onclick="excelExport()" value="导出" style="width:90px"/></security:authorize></td>
						 </tr>
 						<tr>
           ...
           <td style="width:  5%"><div align="center"><span class="STYLE1">${row.carNumber}</span></div></td>
			  ...
			  <c:forEach items="${orderStatus}" var="item">
				<c:if test="${item.dictValue eq row.orderStatus}">
				  <td style="width:  5%"><div align="center"><span class="STYLE1" >${item.dictName}</span></div></td>
				</c:if>
			  </c:forEach>
