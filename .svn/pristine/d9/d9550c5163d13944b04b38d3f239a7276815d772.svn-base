...
import com.boxiang.share.user.dao.MenuRoleDao;
...
public class MenuRoleServiceImpl implements MenuRoleService {
    ...
    @Override
    public MenuRole queryById(java.lang.Integer id) {
     ...
     * 同步选中的权限和数据库中的权限状态
     *
     * @param menuList             权限菜单列表
     * @param checkedMenuCodeArray 用户选中的权限
     * @param userRole             被修改权限的角色
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {RuntimeException.class, Exception.class})
    public void updateRole(List<Menu> menuList, List<String> checkedMenuCodeArray, UserRole userRole) {
     ...
     */
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {RuntimeException.class, Exception.class})
    private void openRole(Menu menu, UserRole userRole) {
        if (menu.getMenuRole() == null) {
            // 不存在该条权限，则新增
            MenuRole mr = new MenuRole();
            mr.setMenuCode(menu.getMenuCode());
            mr.setRoleName(userRole.getRoleName());
            mr.setRolePower(Integer.toString(userRole.getRolePower()));
